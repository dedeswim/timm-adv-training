...
do
  ...
  do
    ...
    do
      ...
      {
        # bsub -o logs/${m}_${a}_${s}_train_%J.out -nnodes 1 -W $BTIME -G $BBANK python -m torch.distributed.run --nproc_per_node=4 --master_port=6712 train.py ${DATA_DIR} --config=$BASE_CONFIG --experiment=$EXPERIMENT_DIR --mean $MEAN --std $STD --model=$m --adv-training=$a --attack-steps=$s
